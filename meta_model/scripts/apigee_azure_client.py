...
def build_payload(model_cfg: dict[str, Any], messages: list[dict[str, str]]) -> dict[str, Any]:
    ...
    # GPT-5.5 through the Mayo Apigee/Azure route may reject or mishandle explicit
    # temperature. Use omit_temperature: true for those deployments. This is needed
    # because the shared YAML defaults may still include temperature: 0.
        ...
        try:
            ...
            if not text:
                raise RuntimeError(
                    ...
                    f"status={resp.status_code}; body={str(data)[:1000]}"
                )
            return text
        except Exception as exc:
            last_error = exc
