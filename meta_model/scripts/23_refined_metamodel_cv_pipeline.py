...
- store annotation decisions separately from span-level semantic evidence;
...
PRESERVED_COLS = ["meaning_preserved", "Results", "results", "expert_meaning_preserved"]
...
DECISION_ARTIFACT_TERMS = set(
    "permit permits permitted permitting deny denies denied denying permission permissions prohibition prohibitions prohibit prohibited obligation obligations required requirement requirements unclear mixed".split()
)
SUGGESTION_STOP = STOP | DECISION_ARTIFACT_TERMS | set("ico fhir duo odrl provision provisions directive role actor action data consent rule".split())
...
DECISION_MARKER_RE = re.compile(r"^\s*\((permit|deny|permission|prohibition|obligation|required|allow|allowed|prohibit|prohibited|mixed|unclear)\)\s*", re.I)
DECISION_FIELDS = ["decision_value", "decision", "permission", "rule_type", "action_type", "polarity"]
...
def tokenize(text: str, extra_stop: set[str] | None = None) -> list[str]:
    stops = STOP | (extra_stop or set())
    return [t for t in re.findall(r"[A-Za-z][A-Za-z0-9_-]*", norm(text).lower()) if t not in stops]


def lexical_head(span: str) -> str:
    toks = tokenize(span, DECISION_ARTIFACT_TERMS)
    return toks[-1] if toks else ""


def canonical_span(span: str) -> str:
    return " ".join(tokenize(span, DECISION_ARTIFACT_TERMS))


def suggestion_terms(texts: list[str], n: int = 12) -> list[str]:
    toks = [t for text in texts for t in tokenize(text, SUGGESTION_STOP)]
    return [x for x, _ in Counter(toks).most_common(n)]
...
def jaccard(a: str, b: str) -> float:
    sa, sb = set(tokenize(a, DECISION_ARTIFACT_TERMS)), set(tokenize(b, DECISION_ARTIFACT_TERMS))
...
def normalize_decision_value(x: Any) -> str:
    ...
    if not s:
        return ""
    if s in {"permit", "permitted", "permission", "permissions", "allow", "allowed", "authorization", "authorize"}:
        return "permit"
    if s in {"deny", "denied", "prohibition", "prohibit", "prohibited", "disallow", "disallowed"}:
        return "deny"
    if s in {"obligation", "obligate", "obligated", "required", "requirement", "must"}:
        return "obligation"
    if s in {"mixed", "both"}:
        return "mixed"
...
def strip_annotation_decision_markers(span: str) -> tuple[str, str, str]:
    """Remove leading annotation-format decisions from a span.

    Example: "(permit) blood" -> ("blood", "permit", "permit").
    These markers are annotation metadata, not source sentence text.
    """
    s = norm(span)
    raw_markers: list[str] = []
    decisions: list[str] = []
    while True:
        m = DECISION_MARKER_RE.match(s)
        if not m:
            break
        raw = m.group(1)
        raw_markers.append(raw)
        dec = normalize_decision_value(raw)
        if dec:
            decisions.append(dec)
        s = s[m.end():]
    clean = norm(s)
    uniq = sorted(set(d for d in decisions if d and d != "unclear"))
    decision = uniq[0] if len(uniq) == 1 else ("mixed" if len(uniq) > 1 else "")
    return clean, decision, ";".join(raw_markers)
...
def annotation_decision(ann: dict[str, Any], marker_decision: str) -> tuple[str, str, str]:
    for k in DECISION_FIELDS:
        if norm(ann.get(k)):
            dec = normalize_decision_value(ann.get(k))
            if dec:
                return dec, k, norm(ann.get(k))
    if marker_decision:
        return marker_decision, "span_marker", marker_decision
    return "", "", ""
...
def make_folds(args: argparse.Namespace) -> None:
    ...
    for i, r in df.iterrows():
        ...
        g["n_rows"] += 1
        g["sentence_hashes"].add(h)
        g["raw_values"].add(raw_form or form)
    ...
    meta = {"n_forms": len(forms), "n_folds": int(args.n_folds), "split_unit": "canonical_consent_form", "seed": int(args.seed), "split_source_csv": str(source_path), "n_raw_form_values": int(sum(r["n_raw_form_values"] for r in forms)), "n_excluded_empty_or_unknown_rows": len(excluded_rows), "note": "Review form_grouping_audit.csv and excluded_empty_form_rows.csv before running fold induction."}
...
def build_mentions(df: pd.DataFrame, fold_map: dict[str, int], test_fold: int, inv: pd.DataFrame) -> pd.DataFrame:
    ...
    preserved_col = pick_col(df, PRESERVED_COLS)
    recon_col = pick_col(df, RECON_COLS)
    ...
    for row_idx, r in df.iterrows():
        ...
        preserved = positive_label(r.get(preserved_col)) if preserved_col else -1
        recon = norm(r.get(recon_col)) if recon_col else ""
        for j, ann in enumerate(parse_annotations(r), start=1):
            uid = get_annotation_uid(ann, info)
            raw_span = get_annotation_span(ann)
            clean_span, marker_decision, marker_raw = strip_annotation_decision_markers(raw_span)
            ann_decision, ann_decision_source, ann_decision_raw = annotation_decision(ann, marker_decision)
            if not uid or not clean_span or uid.lower() in {"na", "n/a", "none"}:
                continue
            ...
            rows.append({
                ...
                "source_element_label": norm(ann.get("source_element_label") or invrow.get("source_element_label") or ann.get("label")),
                "source_element_definition": norm(invrow.get("source_element_definition")),
                "raw_span_text": raw_span,
                "span_text": clean_span,
                "span_decision_marker_raw": marker_raw,
                "annotation_decision": ann_decision,
                "annotation_decision_source": ann_decision_source,
                "annotation_decision_raw": ann_decision_raw,
                "span_canonical": canonical_span(clean_span),
                "span_head": lexical_head(clean_span),
                "span_token_count": len(tokenize(clean_span, DECISION_ARTIFACT_TERMS)),
                "meaning_preserved": preserved,
                "reconstructed_sentence": recon,
                "provenance_key": f"{form}|{sent_id}|{info}|{llm}|row{int(row_idx)+2}|ann{j}",
            })
    return pd.DataFrame(rows)


def write_sentence_decision_evidence(mentions: pd.DataFrame, out_csv: Path) -> None:
    rows = []
    if mentions.empty:
        pd.DataFrame(rows).to_csv(out_csv, index=False)
        return
    for (fold, split, form, sent), g in mentions.groupby(["fold_id", "split", "form_id", "sentence_context_id"], dropna=False):
        vals = [v for v in g["annotation_decision"].astype(str).tolist() if v]
        counts = Counter(vals)
        core = sorted(k for k in counts if k not in {"", "unclear"})
        if len(core) == 1:
            sent_decision = core[0]
        elif len(core) > 1:
            sent_decision = "mixed"
        else:
            sent_decision = "unclear"
        rows.append({"fold_id": fold, "split": split, "form_id": form, "sentence_context_id": sent, "sentence_id": g["sentence_id"].iloc[0], "sentence_text": g["sentence_text"].iloc[0], "sentence_decision": sent_decision, "annotation_decision_counts_json": json.dumps(dict(counts), ensure_ascii=False), "n_annotations": len(g), "n_unique_annotation_decisions": len(counts)})
    pd.DataFrame(rows).to_csv(out_csv, index=False)
...
def sense_nodes(train: pd.DataFrame, min_support: int = 2) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    head_counts = train.groupby(["union_element_id", "span_head"]).size().to_dict()
    rows = []
    for _, r in train.iterrows():
        head = norm(r.get("span_head")) or "no_head"
        uid = norm(r.get("union_element_id"))
        if head_counts.get((uid, head), 0) < min_support:
            head = "misc"
        x = r.to_dict()
        x["sense_key"] = head
        x["sense_id"] = f"{uid}__sense_{re.sub(r'[^A-Za-z0-9]+', '_', head).strip('_') or 'misc'}"
        rows.append(x)
    mentions = pd.DataFrame(rows)
    node_rows = []
    for sid, g in mentions.groupby("sense_id"):
        spans = g["span_text"].astype(str).tolist()
        uids = sorted(g["union_element_id"].astype(str).unique())
        source_models = sorted({u.split("::", 1)[0] for u in uids if "::" in u})
        node_rows.append({
            "sense_id": sid,
            "union_element_ids": json.dumps(uids, ensure_ascii=False),
            "source_models": json.dumps(source_models, ensure_ascii=False),
            "n_mentions": len(g),
            "n_positive_mentions": int((g["meaning_preserved"] == 1).sum()),
            "n_forms": g["form_id"].nunique(),
            "n_sentences": g["sentence_context_id"].nunique(),
            "top_span_heads": json.dumps([x for x, _ in Counter(g["span_head"]).most_common(10) if x], ensure_ascii=False),
            "top_spans": json.dumps([x for x, _ in Counter(spans).most_common(15) if x], ensure_ascii=False),
            "span_head_entropy": entropy(g["span_head"].astype(str).tolist()),
            "source_element_labels": json.dumps([x for x, _ in Counter(g["source_element_label"]).most_common(10) if x], ensure_ascii=False),
        })
    return mentions, pd.DataFrame(node_rows)


def classify_pair(a: pd.Series, b: pd.Series, overlap_threshold: float) -> tuple[str, float]:
    sa, sb = norm(a.get("span_text")), norm(b.get("span_text"))
    ca, cb = norm(a.get("span_canonical")), norm(b.get("span_canonical"))
    ha, hb = norm(a.get("span_head")), norm(b.get("span_head"))
    if not sa or not sb or a.get("sense_id") == b.get("sense_id"):
        return "self_or_empty", 0.0
    jac = jaccard(sa, sb)
    toks_a, toks_b = set(tokenize(sa, DECISION_ARTIFACT_TERMS)), set(tokenize(sb, DECISION_ARTIFACT_TERMS))
    if ca and ca == cb:
...
def build_relationships(sense_mentions: pd.DataFrame, overlap_threshold: float = 0.75) -> tuple[pd.DataFrame, pd.DataFrame]:
    edge_stats: dict[tuple[str, str, str], dict[str, Any]] = {}
    bundle_stats: dict[tuple[str, str], dict[str, Any]] = {}
    ...
    for _, g0 in sense_mentions.groupby("sentence_context_id"):
        ...
        for ra, rb in combinations(g.to_dict("records"), 2):
            ...
            s1, s2 = sorted([str(a["sense_id"]), str(b["sense_id"])])
            rel, ov = classify_pair(a, b, overlap_threshold)
            ...
            d["n_contexts"] += 1
            d["n_positive_contexts"] += pos
            d["max_overlap"] = max(float(d["max_overlap"]), ov)
            d["forms"].add(str(a.get("form_id", "")))
            d["information_models"].update([str(a.get("information_model", "")), str(b.get("information_model", ""))])
            d["llms"].update([str(a.get("llm", "")), str(b.get("llm", ""))])
            ...
            bd["n_cooccurrence_contexts"] += 1
            bd["n_positive_contexts"] += pos
            bd["forms"].add(str(a.get("form_id", "")))
            if len(bd["example_spans"]) < 5:
                bd["example_spans"].append(f"{a.get('span_text')} || {b.get('span_text')}")
    rel_rows = []
    for (s1, s2, rel), d in edge_stats.items():
        hub_penalty = math.sqrt(max(1, freq.get(s1, 1)) * max(1, freq.get(s2, 1)))
        strict = 1.0 if rel == "near_equivalent" else 0.0
        support = float(d["n_positive_contexts"]) + 0.25 * float(d["n_contexts"] - d["n_positive_contexts"])
        weight = strict * support * (1 + 0.05 * max(0, len(d["information_models"]) - 1)) / hub_penalty
...
def equivalence_clusters(nodes: pd.DataFrame, edges: pd.DataFrame, min_weight: float, min_contexts: int) -> pd.DataFrame:
    ...
    node_map = {str(r["sense_id"]): r.to_dict() for _, r in nodes.iterrows()}
    rows = []
    for i, senses in enumerate(sorted(groups.values(), key=lambda xs: (-len(xs), xs[0])), start=1):
        ...
        spans, labels = [], []
        for x in sub:
            spans += json.loads(x.get("top_spans", "[]") or "[]")
            labels += json.loads(x.get("source_element_labels", "[]") or "[]")
        n_mentions = sum(int(x.get("n_mentions", 0)) for x in sub)
        n_pos = sum(int(x.get("n_positive_mentions", 0)) for x in sub)
        source_models = sorted({m for x in sub for m in json.loads(x.get("source_models", "[]") or "[]")})
        rows.append({"candidate_field_id": cid, "sense_ids_json": json.dumps(senses, ensure_ascii=False), "n_sense_nodes": len(senses), "n_mentions": n_mentions, "n_positive_mentions": n_pos, "positive_fraction": round(n_pos / n_mentions, 4) if n_mentions else 0, "source_models_json": json.dumps(source_models, ensure_ascii=False), "top_spans_json": json.dumps([x for x, _ in Counter(spans).most_common(20)], ensure_ascii=False), "source_element_labels_json": json.dumps([x for x, _ in Counter(labels).most_common(20)], ensure_ascii=False), "suggested_terms_json": json.dumps(suggestion_terms(spans), ensure_ascii=False)})
    return pd.DataFrame(rows)
...
def run_fold(args: argparse.Namespace) -> None:
    df = pd.read_csv(args.expert_roundtrips_csv).fillna("")
    ...
    fold_map = {norm(r[fold_col]): int(r["fold_id"]) for _, r in folds.iterrows()}
    inv = load_inventory(Path(args.inventory_csv)) if args.inventory_csv else pd.DataFrame()
    out = Path(args.output_dir) / f"fold_{int(args.fold_id):02d}"
    out.mkdir(parents=True, exist_ok=True)
    mentions = build_mentions(df, fold_map, int(args.fold_id), inv)
    mentions.to_csv(out / "evidence_mentions_all.csv", index=False)
    cleaning = mentions[(mentions.get("raw_span_text", "") != mentions.get("span_text", "")) | mentions.get("span_decision_marker_raw", pd.Series(dtype=str)).astype(str).ne("")].copy() if not mentions.empty else pd.DataFrame()
    cleaning.to_csv(out / "annotation_decision_marker_cleaning_audit.csv", index=False)
    write_sentence_decision_evidence(mentions, out / "sentence_decision_evidence.csv")
    train = mentions[mentions["split"] == "train"].copy() if not mentions.empty else pd.DataFrame()
    test = mentions[mentions["split"] == "test"].copy() if not mentions.empty else pd.DataFrame()
    unassigned = mentions[mentions["split"] == "unassigned"].copy() if not mentions.empty else pd.DataFrame()
    train.to_csv(out / "evidence_mentions_train.csv", index=False)
    test.to_csv(out / "evidence_mentions_test_provenance_only.csv", index=False)
    if not unassigned.empty:
        unassigned.to_csv(out / "evidence_mentions_unassigned_review.csv", index=False)
    sm, nodes = sense_nodes(train, int(args.min_sense_support))
    sm.to_csv(out / "source_element_sense_mentions_train.csv", index=False)
    nodes.to_csv(out / "source_element_sense_nodes.csv", index=False)
    rel, bundle = build_relationships(sm, float(args.span_overlap_threshold))
    rel.to_csv(out / "typed_relationship_edges.csv", index=False)
    bundle.to_csv(out / "provision_bundle_edges.csv", index=False)
    clusters = equivalence_clusters(nodes, rel, float(args.min_equivalence_weight), int(args.min_equivalence_positive_contexts))
    clusters.to_csv(out / "candidate_field_clusters.csv", index=False)
    build_schema(clusters, out / "refined_candidate_schema.yaml", int(args.fold_id), args)
    meta = {"fold_id": int(args.fold_id), "n_train_mentions": int(len(train)), "n_test_mentions_for_provenance": int(len(test)), "n_unassigned_mentions": int(len(unassigned)), "n_cleaned_decision_marker_mentions": int(len(cleaning)), "n_train_forms": int(train["form_id"].nunique()) if not train.empty else 0, "n_test_forms": int(test["form_id"].nunique()) if not test.empty else 0, "n_sense_nodes": int(len(nodes)), "n_relationship_edges": int(len(rel)), "n_bundle_edges": int(len(bundle)), "n_candidate_fields": int(len(clusters))}
    (out / "fold_run_metadata.json").write_text(json.dumps(meta, indent=2))
    print(f"Wrote fold outputs to {out}")
...
def eval_pairs(args: argparse.Namespace) -> None:
    df = pd.read_csv(args.standardized_roundtrips_csv).fillna("")
    orig_col = pick_col(df, ["original_text", "source_text", "canonical_full_text", "full_text"], required=True)
    rec_col = pick_col(df, ["reconstructed_text", "reconstructed_sentence", "backward_mapping"], required=True)
    rows = []
    for _, r in df.iterrows():
        orig = norm(r.get(orig_col))
        rec = norm(r.get(rec_col))
        o, rr = set(content_words(orig)), set(content_words(rec))
        inter = o & rr
        cue_o = {x for x in o if x in CUE_WORDS}
        cue_r = {x for x in rr if x in CUE_WORDS}
...
def main() -> None:
    ...
    p.add_argument("--split_source_csv", default="", help="Use main roundtrips.csv with stable form_key to create folds; defaults to expert_roundtrips_csv.")
    ...
    p.add_argument("--min_sense_support", type=int, default=2)
    p.add_argument("--span_overlap_threshold", type=float, default=0.75)
    p.add_argument("--min_equivalence_weight", type=float, default=0.02)
    p.add_argument("--min_equivalence_positive_contexts", type=int, default=1)
    p.add_argument("--min_field_positive_mentions", type=int, default=5)
    p = sub.add_parser("summarize-folds")
    p.add_argument("--fold_root", required=True)
    p.add_argument("--output_dir", required=True)
    p = sub.add_parser("evaluate-roundtrips")
    p.add_argument("--standardized_roundtrips_csv", required=True)
    p.add_argument("--output_dir", required=True)
    args = ap.parse_args()
    ...
    elif args.cmd == "evaluate-roundtrips":
        eval_pairs(args)
