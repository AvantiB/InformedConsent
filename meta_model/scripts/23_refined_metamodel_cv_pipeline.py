...
This is the paper-facing workflow for reduced meta-model development.

Key safeguards:
- Cross-validation splits are by canonical consent form, never sentence.
- The preferred form identifier is `form_key` from the main roundtrips.csv.
- Expert workbook rows are mapped to the same canonical form IDs by form_key when
  present and by conservative filename cleanup otherwise.
- Source elements are first split into context-specific sense nodes.
- Only strict near-equivalence edges merge candidate fields.
- Co-occurrence is retained as provision-bundle/complementarity evidence only.
- Outputs retain form, row, sentence, source-model, LLM, and span provenance.
"""
...
def raw_form_value(row: pd.Series) -> str:
    for c in FORM_COLS:
        if c in row.index and norm(row.get(c)):
            return norm(row.get(c))
    return "form_unknown_" + stable_id(norm(row.to_json()), 8)


def canonical_form_id(value: str) -> str:
    """Map copy/output filenames and stable form_key values to one form ID."""
    s = norm(value)
    if not s:
        return "form_unknown"
    s = s.replace("\\", "/").split("/")[-1]
    s = re.sub(r"\.(txt|csv|xlsx?|json)$", "", s, flags=re.I).strip()
    s = re.sub(r"\s+", " ", s)
    s = re.sub(r"(?i)([_\s-]+(?:output|annotated|annotation|merged|roundtrip))+$", "", s).strip(" _-")
    s = re.sub(r"(?i)\s+copy[_\s-]*\d+.*$", "", s).strip(" _-")
    s = re.sub(r"(?i)_copy[_\s-]*\d+.*$", "", s).strip(" _-")
    s = re.sub(r"(?i)[_\s-]+copy\b.*$", "", s).strip(" _-")
    s = re.sub(r"(?i)[_\s-]*(chatgpt|gpt5|gpt-5|qwen|medgemma|llama).*$", "", s).strip(" _-")
    s = re.sub(r"_+", "_", s).strip(" _-")
    return s or "form_unknown_" + stable_id(value, 8)
...
def make_folds(args: argparse.Namespace) -> None:
    split_path = Path(args.split_source_csv or args.expert_roundtrips_csv)
    df = pd.read_csv(split_path).fillna("")
    text_col = pick_col(df, TEXT_COLS, required=False)
    groups: dict[str, dict[str, Any]] = {}
    audit_rows = []
    for idx, r in df.iterrows():
        raw = raw_form_value(r)
        form = canonical_form_id(raw)
        text = norm(r.get(text_col)) if text_col else ""
        audit_rows.append({"source_csv": str(split_path), "source_row": int(idx) + 2, "raw_form_value": raw, "canonical_form_id": form, "sentence_id": source_sentence_id(r, text), "sentence_text_hash": stable_id(text) if text else ""})
        g = groups.setdefault(form, {"form_id": form, "canonical_form_id": form, "raw_form_values": set(), "n_rows": 0, "sentence_hashes": set()})
        g["raw_form_values"].add(raw)
        g["n_rows"] += 1
        if text:
            g["sentence_hashes"].add(stable_id(text))
    forms = []
    for form, g in groups.items():
        forms.append({
            "form_id": form,
            "canonical_form_id": form,
            "n_rows": int(g["n_rows"]),
            "n_unique_sentences": len(g["sentence_hashes"]),
            "n_raw_form_values": len(g["raw_form_values"]),
            "raw_form_values_json": json.dumps(sorted(g["raw_form_values"]), ensure_ascii=False),
        })
    ...
    meta = {"n_forms": len(forms), "n_folds": int(args.n_folds), "split_unit": "canonical_consent_form", "seed": int(args.seed), "split_source_csv": str(split_path), "n_raw_form_values": len({x["raw_form_value"] for x in audit_rows}), "note": "Review form_grouping_audit.csv before running fold induction."}
    (out / "fold_metadata.json").write_text(json.dumps(meta, indent=2))
    print(f"Wrote {out / 'fold_assignments.csv'} with {len(forms)} canonical forms")
...
def build_mentions(df: pd.DataFrame, fold_map: dict[str, int], test_fold: int, inv: pd.DataFrame) -> pd.DataFrame:
    ...
    for row_idx, r in df.iterrows():
        ...
        raw_form = raw_form_value(r)
        form = canonical_form_id(raw_form)
        fold = fold_map.get(form)
        split = "test" if fold == test_fold else "train" if fold is not None else "unassigned"
        ...
        for j, ann in enumerate(parse_annotations(r), start=1):
            ...
            rows.append({
                "fold_id": test_fold, "split": split, "form_id": form, "raw_form_value": raw_form, "source_row": int(row_idx) + 2,
                "sentence_id": sent_id, "sentence_context_id": context_id, "sentence_text": text,
                "information_model": info, "llm": llm, "annotation_index": j,
                ...
                "span_text": span, "span_canonical": canonical_span(span), "span_head": lexical_head(span), "span_token_count": len(tokenize(span)),
                "sentence_decision": norm(ann.get("decision_value") or ann.get("decision") or r.get("roundtrip_decision", "")),
                "meaning_preserved": preserved, "reconstructed_sentence": recon,
...
def sense_nodes(train: pd.DataFrame, min_support: int = 2) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for sid, g in mentions.groupby("sense_id"):
        ...
        node_rows.append({
            "sense_id": sid, "union_element_ids": json.dumps(uids, ensure_ascii=False), "source_models": json.dumps(source_models, ensure_ascii=False),
            "n_mentions": len(g), "n_positive_mentions": int((g["meaning_preserved"] == 1).sum()), "n_forms": g["form_id"].nunique(), "n_sentences": g["sentence_context_id"].nunique(),
...
def build_relationships(sense_mentions: pd.DataFrame, overlap_threshold: float = 0.75) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, g0 in sense_mentions.groupby("sentence_context_id"):
        ...
        for ra, rb in combinations(g.to_dict("records"), 2):
            ...
            d["n_contexts"] += 1; d["n_positive_contexts"] += pos; d["max_overlap"] = max(float(d["max_overlap"]), ov)
            d["forms"].add(str(a.get("form_id", ""))); d["information_models"].update([str(a.get("information_model", "")), str(b.get("information_model", ""))]); d["llms"].update([str(a.get("llm", "")), str(b.get("llm", ""))])
...
def equivalence_clusters(nodes: pd.DataFrame, edges: pd.DataFrame, min_weight: float, min_contexts: int) -> pd.DataFrame:
    ...
    for i, senses in enumerate(sorted(groups.values(), key=lambda xs: (-len(xs), xs[0])), start=1):
        cid = f"F{i:03d}"; sub = [node_map[s] for s in senses if s in node_map]
        spans, labels = [], []
        for x in sub:
            spans += json.loads(x.get("top_spans", "[]") or "[]"); labels += json.loads(x.get("source_element_labels", "[]") or "[]")
        n_mentions = sum(int(x.get("n_mentions", 0)) for x in sub); n_pos = sum(int(x.get("n_positive_mentions", 0)) for x in sub)
...
def run_fold(args: argparse.Namespace) -> None:
    ...
    out = Path(args.output_dir) / f"fold_{int(args.fold_id):02d}"; out.mkdir(parents=True, exist_ok=True)
...
def eval_pairs(args: argparse.Namespace) -> None:
    ...
    orig_col = pick_col(df, ["original_text", "source_text", "canonical_full_text", "full_text"], required=True); rec_col = pick_col(df, ["reconstructed_text", "reconstructed_sentence", "backward_mapping"], required=True)
    rows = []
    for _, r in df.iterrows():
