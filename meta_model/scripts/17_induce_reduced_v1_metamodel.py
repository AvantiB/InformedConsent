...
This script is the empirical discovery step. It does not hard-code the V1
fields and it does not automatically turn co-occurrence bundles into fields.
It writes two separate graph views:

1. semantic-equivalence graph: source-model elements that may express the same
   semantic field, based on same/overlapping spans, cross-model support,
   expert-positive support, and profile similarity;
2. provision-bundle graph: source-model elements that co-occur compositionally
   in the same sentence/model run. This is used to understand provision
   structure, not to merge fields.

The final V1 schema should be built only after auditing the semantic clusters.
"""
...
def toks(x:str)->set[str]: return set(re.findall(r"[a-z0-9]+",x.lower()))-STOP
...
def annotations_from_cell(raw:Any, info:str)->list[dict[str,Any]]:
    ...
    if not anns:
        for m in BRACKET_RE.finditer(str(raw or "").replace("\r","\n")):
            uid=canon(norm(m.group("label")),info)
            if uid:
    ...
    out=[]; seen=set()
    for a in anns:
        raw_id=first(a,ELEM_KEYS); uid=canon(raw_id,info)
        if not uid: continue
...
def build_mentions(df:pd.DataFrame,args)->tuple[pd.DataFrame,pd.DataFrame]:
    ...
    for idx,r in df.iterrows():
        ...
        info=norm(r.get(mc,"")) if mc else "unknown"; llm=norm(r.get(lc,"")) if lc else "unknown"
        context_id=f"{sentence_key}|{info}|{llm}|{idx}"
        for j,a in enumerate(annotations_from_cell(r.get(ac,""),info)):
            uid=norm(a["union_element_id"])
            rec={"row_index":idx,"context_id":context_id,"sentence_key":sentence_key,"source_id":source_id,"source_text":text,"information_model":info,"llm":llm,"expert_meaning_preserved":bool(ok),"union_element_id":uid,"source_model_inferred":src_model(uid,info),"span_text":norm(a.get("span_text","")),"span_norm":" ".join(sorted(toks(a.get("span_text","")))),"raw_element_label":norm(a.get("source_element_label","")),"decision_value":norm(a.get("decision_value","")),"annotation_index":j}
...
def semantic_edges(m:pd.DataFrame, prof:pd.DataFrame, min_w:float, span_jacc:float)->pd.DataFrame:
    ...
    pmap={r.union_element_id:r for _,r in prof.iterrows()}; bucket=defaultdict(lambda:{"same_span":set(),"overlap_span":set(),"fail_same":set(),"llms":set(),"infos":set()})
    pos=m[m.expert_meaning_preserved.astype(bool)].copy(); neg=m[~m.expert_meaning_preserved.astype(bool)].copy()
    for sk,g in pos.groupby("sentence_key"):
        rows=list(g.to_dict("records"))
        for a,b in itertools.combinations(rows,2):
            if a["union_element_id"]==b["union_element_id"]: continue
            sa,sb=toks(a["span_text"]),toks(b["span_text"])
            same=norm(a["span_text"]).lower()==norm(b["span_text"]).lower() and norm(a["span_text"])
            ov=jac(sa,sb)>=span_jacc if sa and sb else False
            if not (same or ov): continue
            rec=bucket[tuple(sorted((a["union_element_id"],b["union_element_id"]))))]
            rec["same_span" if same else "overlap_span"].add(sk); rec["llms"].update([a["llm"],b["llm"]]); rec["infos"].update([a["information_model"],b["information_model"]])
    for sk,g in neg.groupby("sentence_key"):
        for a,b in itertools.combinations(g.to_dict("records"),2):
            if a["union_element_id"]!=b["union_element_id"] and norm(a["span_text"]).lower()==norm(b["span_text"]).lower() and norm(a["span_text"]): bucket[tuple(sorted((a["union_element_id"],b["union_element_id"])))]["fail_same"].add(sk)
    rows=[]; ids=prof.union_element_id.astype(str).tolist()
    for a,b in itertools.combinations(ids,2):
        rec=bucket.get(tuple(sorted((a,b))),{"same_span":set(),"overlap_span":set(),"fail_same":set(),"llms":set(),"infos":set()})
        sim=jac(toks(str(pmap[a].profile_text)),toks(str(pmap[b].profile_text)))
        same,over,fail=len(rec["same_span"]),len(rec["overlap_span"]),len(rec["fail_same"])
...
def bundle_edges(m:pd.DataFrame)->pd.DataFrame:
    if m.empty: return pd.DataFrame()
    ...
    rows=[]
    for (a,b),rec in bucket.items():
        rows.append({"union_element_id_a":a,"union_element_id_b":b,"positive_cooccurrence_contexts":len(rec["pos"]),"negative_cooccurrence_contexts":len(rec["neg"]),"raw_joint_positive_mention_intensity":rec["raw_pos"],"edge_type":"provision_bundle_not_merge"})
...
def cluster_edges(prof:pd.DataFrame, edges:pd.DataFrame)->pd.DataFrame:
...
def cluster_summary(clust:pd.DataFrame, min_sent:int)->pd.DataFrame:
    ...
    for cid,g in clust.groupby("semantic_cluster_id"):
        ...
        if ps>=min_sent and pim>=2 and pll>=2 and rate>=.5: status="high_support_equivalence_cluster"
        elif ps>=max(3,min_sent//3) and rate>=.5: status="context_specific_equivalence_cluster"
        else: status="single_source_or_low_support_cluster"
...
def bundle_by_cluster(bundle:pd.DataFrame, clust:pd.DataFrame)->pd.DataFrame:
    ...
    for _,e in bundle.iterrows():
        ...
        if ca and cb and ca!=cb:
            rows.append({"semantic_cluster_id_a":ca,"semantic_cluster_id_b":cb,"element_a":e.union_element_id_a,"element_b":e.union_element_id_b,"positive_cooccurrence_contexts":e.positive_cooccurrence_contexts,"negative_cooccurrence_contexts":e.negative_cooccurrence_contexts,"raw_joint_positive_mention_intensity":e.raw_joint_positive_mention_intensity})
...
def main()->None:
    ...
    mentions,decisions=build_mentions(df,args); prof=profiles(mentions,inv); sedges=semantic_edges(mentions,prof,args.min_semantic_edge_weight,args.span_overlap_jaccard); bundles=bundle_edges(mentions); clust=cluster_edges(prof,sedges); cs=cluster_summary(clust,args.min_core_positive_sentences); ds=decision_summary(decisions); bc=bundle_by_cluster(bundles,clust); audit=audit_template(cs)
    ...
    write_method(out,args); write_md(out,cs); print(f"Wrote empirical V1 discovery outputs to {out}")
