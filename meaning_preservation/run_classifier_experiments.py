...
def add_embeddings(df: pd.DataFrame, feats: pd.DataFrame, model_name: str | None, batch_size: int) -> pd.DataFrame:
    if not model_name:
        return feats
    try:
        from sentence_transformers import SentenceTransformer
    except Exception as e:
        print(f"[WARN] sentence-transformers unavailable; skipping embeddings: {e}")
        return feats
    ...
    feats = feats.copy()
    feats["embedding_cosine"] = (eo * er).sum(axis=1)
...
def run_experiments(df: pd.DataFrame, feats: pd.DataFrame, out_dir: Path, seed: int, n_folds: int, run_rf: bool):
    ...
    for name, tr, te in sp:
        ...
        for mname, model in models.items():
            ...
            row = {"split": name, "model": mname, "n_train": len(tr), "n_test": len(te)}; row.update(metrics(y[te], p)); rows.append(row)
            for t in [0.5, 0.7, 0.8, 0.9]:
                pred = p >= t
                th_rows.append({"split": name, "model": mname, "threshold": t, "n_retained": int(pred.sum()), "retained_fraction": pred.mean(), "precision_at_threshold": precision_score(y[te], pred, zero_division=0), "recall_at_threshold": recall_score(y[te], pred, zero_division=0)})
        bow = Pipeline([("tfidf", TfidfVectorizer(max_features=5000, ngram_range=(1, 2), min_df=1)), ("clf", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear"))])
        bow.fit(df.iloc[tr]["pair_text"], y[tr]); p = scores(bow, df.iloc[te]["pair_text"])
        row = {"split": name, "model": "bow_tfidf_lr", "n_train": len(tr), "n_test": len(te)}; row.update(metrics(y[te], p)); rows.append(row)
        ...
        row = {"split": name, "model": "hybrid_bow_features_lr", "n_train": len(tr), "n_test": len(te)}; row.update(metrics(y[te], p)); rows.append(row)
        for t in [0.5, 0.7, 0.8, 0.9]:
            pred = p >= t
            th_rows.append({"split": name, "model": "hybrid_bow_features_lr", "threshold": t, "n_retained": int(pred.sum()), "retained_fraction": pred.mean(), "precision_at_threshold": precision_score(y[te], pred, zero_division=0), "recall_at_threshold": recall_score(y[te], pred, zero_division=0)})
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    final = Pipeline([("pre", pre), ("clf", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear"))]).fit(Xf, y)
    joblib.dump(final, out_dir / "final_feature_lr.joblib")
    try:
        imp = pd.DataFrame({"feature": final.named_steps["pre"].get_feature_names_out(), "coefficient": final.named_steps["clf"].coef_.ravel()})
...
def write_report(processed: Path, results: Path):
    ...
    lines = ["# Meaning-Preservation Classifier MVP Report", "", "## Dataset audit", audit.to_markdown(index=False), "", "## Metrics", met.sort_values(["split", "f1"], ascending=[True, False]).to_markdown(index=False), ""]
    ...
    lines += ["## Note", "Use this classifier as a proxy evaluator for binary human meaning-preservation labels, not as definitive ontology-correctness validation."]
...
def main():
    ...
    args = ap.parse_args()
    out = Path(args.output_dir); processed = out / "processed"; features = out / "features"; results = out / "results"
    for d in [processed, features, results]: d.mkdir(parents=True, exist_ok=True)
    ...
    feats = add_embeddings(df, feats, args.embedding_model, args.batch_size)
