...
RESTRICTION = [
    "only", "limited", "limit", "restriction", "restricted", "commercial", "non-commercial",
    "identifiable", "de-identified", "geographic", "institution", "approved", "irb", "ethics",
    "no expiration", "at any time",
]
WITHDRAWAL = ["withdraw", "revoke", "quit", "stop", "withdrawal"]
ACTIONS = [
    "use", "used", "store", "stored", "share", "shared", "disclose", "disclosed", "collect",
    "collected", "withdraw", "revoke", "destroy", "retain", "contact", "return", "access",
    "sell", "distribute", "retrieve", "study", "analyze", "learn",
]
RESOURCES = [
    "data", "information", "health information", "medical record", "records", "dna", "sample",
    "samples", "specimen", "specimens", "biospecimen", "blood", "urine", "saliva", "results",
    "database", "databases",
]
ACTORS = [
    "researcher", "researchers", "doctor", "doctors", "study team", "institution", "sponsor",
    "company", "biobank", "irb", "university", "clinic", "hospital", "all of us", "mayo",
]
...
EMBEDDING_COLS = ["embedding_cosine", "embedding_distance"]
...
def make_preprocessor(feats: pd.DataFrame, include_metadata: bool = True):
...
def _threshold_rows(split_name: str, model_name: str, y_true: np.ndarray, p: np.ndarray) -> list[dict]:
    rows = []
...
def _fit_eval(model, X_train, y_train, X_test, y_test, split_name: str, model_name: str):
    fit = model.fit(X_train, y_train)
    p = scores(fit, X_test)
    row = {"split": split_name, "model": model_name, "n_train": len(y_train), "n_test": len(y_test)}
    row.update(metrics(y_test, p))
    return row, _threshold_rows(split_name, model_name, y_test, p)
...
def run_experiments(df: pd.DataFrame, feats: pd.DataFrame, out_dir: Path, seed: int, n_folds: int, run_rf: bool):
    ...
    embedding_cols = [c for c in EMBEDDING_COLS if c in feats.columns]
    ...
    for name, tr, te in sp:
        ...
        ytr, yte = y[tr], y[te]
        models = {
            "majority": (DummyClassifier(strategy="most_frequent"), Xf),
            "feature_lr": (Pipeline([("pre", pre), ("clf", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear"))]), Xf),
            "feature_lr_no_metadata": (Pipeline([("pre", pre_no), ("clf", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear"))]), Xf_no),
        }
        if run_rf:
            models["feature_rf"] = (Pipeline([("pre", pre), ("clf", RandomForestClassifier(n_estimators=300, random_state=seed, class_weight="balanced"))]), Xf)

        for mname, (model, X) in models.items():
            row, extra = _fit_eval(model, X.iloc[tr], ytr, X.iloc[te], yte, name, mname)
            rows.append(row)
            th_rows.extend(extra)

        if embedding_cols:
            Xemb = feats[embedding_cols]
            emb_model = Pipeline([
                ("imp", SimpleImputer(strategy="median")),
                ("sc", StandardScaler()),
                ("clf", LogisticRegression(max_iter=2000, class_weight="balanced", solver="liblinear")),
            ])
            row, extra = _fit_eval(emb_model, Xemb.iloc[tr], ytr, Xemb.iloc[te], yte, name, "embedding_only_lr")
            rows.append(row)
            th_rows.extend(extra)
        ...
        if nli_cols:
            ...
            row, extra = _fit_eval(nli_model, Xnli.iloc[tr], ytr, Xnli.iloc[te], yte, name, "nli_only_lr")
            rows.append(row)
            th_rows.extend(extra)
        ...
        row, extra = _fit_eval(bow, df.iloc[tr]["pair_text"], ytr, df.iloc[te]["pair_text"], yte, name, "bow_tfidf_lr")
        rows.append(row)
        th_rows.extend(extra)
        ...
        Xc_tr, Xc_te = pre.fit_transform(Xf.iloc[tr], ytr), pre.transform(Xf.iloc[te])
        clf = LogisticRegression(max_iter=3000, class_weight="balanced", solver="liblinear")
        clf.fit(sparse.hstack([Xb_tr, Xc_tr]).tocsr(), ytr)
        p = scores(clf, sparse.hstack([Xb_te, Xc_te]).tocsr())
        row = {"split": name, "model": "hybrid_bow_features_lr", "n_train": len(tr), "n_test": len(te)}
        row.update(metrics(yte, p))
        rows.append(row)
        th_rows.extend(_threshold_rows(name, "hybrid_bow_features_lr", yte, p))
...
def write_run_metadata(results_dir: Path, args, feats: pd.DataFrame):
    embedding_cols = [c for c in EMBEDDING_COLS if c in feats.columns]
    nli_cols = [c for c in feats.columns if c.startswith("nli_")]
    numeric_cols = [c for c in feats.columns if pd.api.types.is_numeric_dtype(feats[c])]
    metadata = {
        "roundtrips_csv": str(args.roundtrips_csv),
        ...
        "nli_columns": nli_cols,
        "numeric_feature_count": len(numeric_cols),
        "all_numeric_features_include_semantic_features": bool(embedding_cols or nli_cols),
        "note": "feature_lr and hybrid_bow_features_lr include all numeric features present, including embedding/NLI features when generated. embedding_only_lr isolates embedding_cosine and embedding_distance.",
    }
...
def write_report(processed: Path, results: Path):
    ...
    lines = ["# Meaning-Preservation Classifier MVP Report", ""]
    if metadata:
        show_keys = [
            "embedding_model_requested", "embedding_model_resolved", "embedding_backend", "embedding_device",
            "embedding_features_present", "embedding_columns", "nli_model_requested", "nli_features_present",
            "nli_columns", "numeric_feature_count",
        ]
        meta_rows = []
        for k in show_keys:
            v = metadata.get(k, "")
            ...
            meta_rows.append({"field": k, "value": v})
        lines += ["## Run metadata", pd.DataFrame(meta_rows).to_markdown(index=False), ""]
...
def main():
    ...
    write_run_metadata(results, args, feats)
